...
void kernel_main(multiboot_info_t* mbi) {
    idt_init();
    fs_init();
    print_colored("File system initialized.\n", 0x04);
    timer_init();
    keyboard_init();

    clear_screen();
    print_colored("Welcome to gos OS\n", 0x04);
    print_colored("Enter Passkey Pretty Please: ", 0x04);

    char* passkey = malloc(420);

    if (strcmp(passkey, "letmein") == 0) {
        print_colored("\nPasskey correct YAYYY!\nLaunching shell...\n", 0x04);
        if (mbi->flags & (1 << 3)) {
